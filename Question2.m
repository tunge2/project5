...
cam_def; %define camera
target_def; % define target points in the inertial frame
%M=10; % Specify # of images
M = 10;
L=zeros(2*M,6);% calibration fit matrix

for i= 1:M % build up the calibration fit matrix
    image_gen_flag = 1;
    while image_gen_flag > 0
        ...
        p=[1*(rand-.5);1*(rand-.5);-(rand+1)*2]; % trying to increase grid spread
    ...
    % ns pixel error
    cam.ns=.1;
    uv_meas{i}=uv_true{i}+randn(size(uv_true{i}))*cam.ns;
    
    % find the Homography matrix
    Hmat{i}=homographymat(uv_meas{i},Pxy);
    hx=Hmat{i}(:,1); %first column 
    hy=Hmat{i}(:,2); %second column
...
Best=[Bvec(1) Bvec(4) Bvec(5);...
    Bvec(4) Bvec(2) Bvec(6);...
    Bvec(5) Bvec(6) Bvec(3)];

K_est=inv(chol(Best*Best(3,3)));
...
figure(2000);
for i=1:M
  plot(uv_true{i}(1,:),uv_true{i}(2,:),'.',...
      uv_est{i}(1,:),uv_est{i}(2,:),'o','linewidth',0.5);
  hold on
end
