...
for i=1:M
    ...
    while full_image_in_frame_flag == 0
        ...
        uv_noise{i}=uv{i}+randn(size(uv{i}))*cam.ns;
        %disp(size(uv{i},2));
        % check if image is completely in frame, if not, continue
        if size(uv_noise{i},2) == size(pS,2)
    ...
    [T_C0_est,Zest]=pnp_general(uv_noise{i},pS,cam.K);
...
disp('p_TC vs. estimate (m)');
disp([P_TC P_TC_est]);

disp('p_TC vs. estimate Error (mm)');
A = abs(P_TC-P_TC_est)*1000;
disp([A]);

disp('p_0B vs. estimate (m)');
disp([P_0B P_0B_est]);

disp('p_0B vs. estimate Error (mm)');
B = abs(P_0B-P_0B_est)*1000;
disp([B]);
